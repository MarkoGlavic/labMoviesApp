...
export const getPeople = (args) => {
  const [,page]=args.queryKey
  return fetch(
    `https://api.themoviedb.org/3/person/popular?api_key=${process.env.REACT_APP_TMDB_KEY}&language=en-US&page=${page}&include_adult=false`
    ).then((response) => {
    if (!response.ok) {
      throw new Error(response.json().message);
    }
...
export const getShows = (args) => {
  ...
  const [,page]=args.queryKey
  return fetch(
    `https://api.themoviedb.org/3/discover/tv?api_key=${process.env.REACT_APP_TMDB_KEY}&language=en-US&include_video=false&page=${page}`
    ).then((response) => {
    if (!response.ok) {
      throw new Error(response.json().message);
    }
...
export const getShow = (args) => {
  ...
  export const getMovieByPerson = (args) => {
    // console.log(args)
    const [, idPart] = args.queryKey;
    const { id } = idPart;
    return fetch(
  ...
  export const getUpcomingMovie = () => {
    return fetch(
      `https://api.themoviedb.org/3/movie/upcoming?api_key=${process.env.REACT_APP_TMDB_KEY}&language=en-us&page1`
    ).then((response) => {
      if (!response.ok) {
        throw new Error(response.json().message);
      }
